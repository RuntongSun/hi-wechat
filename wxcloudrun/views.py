...
@app.route('/upload_voice_from_url', methods=['POST'])
def upload_voice_from_url():
    voice_url = request.form.get("voice_url")
    open_id = request.form.get("touser")

    if not voice_url or not open_id:
        return 'No image URL or touser in the request', 400

    upload_result = wechat_manager.upload_voice_from_oss(voice_url)
